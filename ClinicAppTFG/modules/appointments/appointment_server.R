  ...
  moduleServer(id, function(input, output, session){
    ...
    output$ui_filtro_doctor <- renderUI({
      ...
      docs <- DBI::dbGetQuery(
        ...
        "SELECT id, nombre 
         FROM usuarios 
         WHERE tipo_usuario='doctor' OR tipo_usuario='higienista'"
      )
    ...
    hay_conflicto <- function(pool, doctor, gabinete, inicio, fin, cita_id=NULL){
      ...
      inicio_str <- format(inicio,"%Y-%m-%d %H:%M:%S", tz="Europe/Madrid")
      fin_str    <- format(fin,"%Y-%m-%d %H:%M:%S", tz="Europe/Madrid")
    ...
    output$cal <- renderCalendar({
      ...
      df <- if(length(params)>0){
        ...
        DBI::dbGetQuery(pool, query_base)
      }
        ...
        modalDialog(
          ...
          div(class="container-fluid",
              ...
              div(class="row",
                  ...
                  div(class="col-12 col-sm-6",
                      selectInput(
                        ...
                        selected=if(editar) datos$paciente_id
                      )
                  ),
                  ...
                  div(class="col-12 col-sm-6",
                      selectInput(
                        ...
                        selected=if(editar) datos$profesional_id
                      )
                  )
              ),
              ...
              div(class="row",
                  ...
                  div(class="col-12 col-sm-6",
                      numericInput(
                        ns("gabinete"),
                        "Gabinete",
                        value=if(editar) datos$gabinete else 1,
                        min=1,
                        max=3
                      )
                  ),
                  ...
                  div(class="col-12 col-sm-6",
                      textInput(
                        ns("servicio"),
                        "Servicio",
                        value=if(editar) datos$tipo_servicio else "Revisión"
                      )
                  )
              ),
              ...
              numericInput(
                ...
                "Duración (min)",
                value=if(editar)
                  as.numeric(difftime(rv$end, rv$start, units="mins"))
                else 60,
                step=15
              )
          ),
          ...
          footer = tagList(
            ...
            if(editar)
              actionButton(ns("borrar"),"Eliminar",
                           class="btn-danger",
                           style="float:left"),
    ...
    observeEvent(input$guardar,{
      ...
      if(!is.null(conflictos)){
        ...
        showNotification(
          "⚠️ Error: El doctor o el gabinete ya tienen una cita en este horario.",
          type="error"
        )
      ...
      t_start_sql <- format(t_start_dt,"%Y-%m-%d %H:%M:%S", tz="Europe/Madrid")
      t_end_sql   <- format(t_end_dt,"%Y-%m-%d %H:%M:%S", tz="Europe/Madrid")
